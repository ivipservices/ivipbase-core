...
export interface EventSettings {
    /**
     * Especifica se deve pular callbacks para o valor atual (aplica-se aos eventos `"value"` e `"child_added"`)
     */
    newOnly?: boolean;
    /**
     * Permite implementar lógica de sincronização personalizada se a sincronização entre cliente e servidor não puder ser feita
     * automaticamente para este evento. Por exemplo, este callback será executado para um evento "child_changed" que
     * foi adicionado offline e só disparado para alterações no cache local até que o servidor esteja conectado; se não houver uma assinatura de evento "value"
     * ativa no mesmo caminho, você deve atualizar manualmente o estado local carregando dados atualizados do servidor. Definir essa propriedade como "reload" fará isso automaticamente.
     */
...
export class DataRetrievalOptions {
    /**
     * chaves filhas a serem incluídas (excluirão outras chaves), podem incluir curingas (por exemplo, "messages/*\/title")
     */
    include?: Array<string|number>;

    /**
     * chaves filhas a serem excluídas (incluirão outras chaves), podem incluir curingas (por exemplo, "messages/*\/respostas")
     */
    exclude?: Array<string|number>;

    /**
     * se deve ou não incluir quaisquer objetos filho, o padrão é verdadeiro
     */
    child_objects?: boolean;

    /**
     * Se um valor em cache pode ser utilizado. Um valor em cache será usado se o cliente estiver offline, se a configuração de prioridade do cache estiver definida como verdadeira, ou se o valor em cache estiver disponível e o valor do servidor levar muito tempo para carregar (>1s). Se o valor solicitado não estiver filtrado, o cache será atualizado com o valor recebido do servidor, acionando quaisquer listeners de evento definidos no caminho. O valor padrão é true.
     * @deprecated allow_cache: "allow" instead
     * @default true
     */
    allow_cache?: boolean;
