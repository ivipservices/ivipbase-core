import {
	...
	ArrayReduceMethod,
	ILiveDataProxy,
	ILiveDataProxyTransaction,
	ILiveDataProxyValue,
	IProxyContext,
	LiveDataProxyOptions,
	ProxyObserveError,
...
export function proxyAccess<ValueType>(proxiedValue: ValueType): ILiveDataProxyValue<ValueType> {
	if (typeof proxiedValue !== "object" || !(proxiedValue as any)[isProxy]) {
...
export class LiveDataProxy {
	...
	static async create<T>(ref: DataReference, options?: LiveDataProxyOptions<T>): Promise<ILiveDataProxy<T>> {
		...
		let proxy: ILiveDataProxyValue<T> | null;
		...
		const pushLocalMutations = async () => {
			...
			process.nextTick(() => {
				...
				const snap = new MutationsDataSnapshot(
					ref,
			...
			const mutationsHandler = async (details: { snap: MutationsDataSnapshot; origin: "remote" | "local" }) => {
				...
				process.nextTick(() => {
					...
					try {
						...
						clientEventEmitter.emit("error", <ProxyObserveError>{
		...
		const handleFlag = (flag: "write" | "onChange" | "subscribe" | "observe" | "transaction", target: Array<string | number>, args: any) => {
			if (flag === "write") {
				...
				return new Promise(async (resolve) => {
					...
					const tx: { target: RelativeNodeTarget; status: "started" | "finished" | "canceled"; transaction: ILiveDataProxyTransaction | null } = {
						target,
						status: "started",
						transaction: null,
					};
...
export class OrderedCollectionProxy<ItemType extends { [KeyName in OrderKeyName]: number }, OrderKeyName extends string = "order"> {
	constructor(private collection: ObjectCollection<ItemType & { [KeyName in OrderKeyName]: number }>, private orderProperty = "order" as OrderKeyName, private orderIncrement: number = 10) {
		if (typeof collection !== "object" || !(collection as any)[isProxy]) {
...
function createProxy<T>(context: {
	root: { ref: DataReference; readonly cache: any };
	target: RelativeNodeTarget;
	id: string;
	flag(flag: "write" | "onChange" | "subscribe" | "observe" | "transaction", target: RelativeNodeTarget, args?: any): any;
}): ILiveDataProxyValue<T> {
	...
	const handler: ProxyHandler<any> = {
		get(target, prop: string | symbol | number, receiver) {
			...
			const unproxyValue = (value: any) => {
				return value !== null && typeof value === "object" && value[isProxy] ? (value as ILiveDataProxyValue<any>).getTarget() : value;
			};
			...
			if (typeof value === "undefined") {
				...
				if (isArray) {
					...
					const cleanArrayValues = (values: any) =>
