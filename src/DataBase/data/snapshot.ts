...
function getChild<T extends DataSnapshot = DataSnapshot>(
    snapshot: T,
    path: string | number,
    previous = false
) {
...
function getChildren<T extends DataSnapshot = DataSnapshot>(
    snapshot: T
): Array<string | number> {
    if (!snapshot.exists()) {
        return [];
...
export class DataSnapshot<T = any> {
    ...
    child<ChildType = any>(
        path: string
    ): ChildType extends keyof T
        ? DataSnapshot<T[ChildType]>
        : DataSnapshot<ChildType>;
     ...
     */
    forEach<Child extends DataSnapshot = DataSnapshot<T[keyof T]>>(
        callback: (child: Child) => boolean
    ): boolean {
...
export class MutationsDataSnapshot<
    Value = any,
    PrevValue = Value,
    T extends IDataMutationsArray<Value, PrevValue> = IDataMutationsArray<
        Value,
        PrevValue
    >
> extends DataSnapshot<T> {
    /**
    ...
    previous = () => {
        throw new Error(
            "Iterate values to get previous values for each mutation"
        );
    };
     ...
     */
    forEach<Child extends DataSnapshot = DataSnapshot>(
        callback: (child: Child) => boolean
    ): boolean {
        const mutations = this.val(false);
        return mutations.every((mutation) => {
            const ref = mutation.target.reduce(
                (ref, key) => ref.child(key),
                this.ref
            );
            const snap = new DataSnapshot(
                ref,
