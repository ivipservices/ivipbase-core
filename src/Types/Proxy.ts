...
export type ProxyObserveMutation = { snapshot: DataSnapshot; isRemote: boolean };
export type ProxyObserveMutationsCallback = (mutationSnapshot: DataSnapshot, isRemoteChange: boolean) => any;

export type ProxyObserveError = { source: string; message: string; details: Error };
 ...
 */
export type ProxySubscribeFunction<T> = (observer: { next: (val: T) => void }) => () => void;
