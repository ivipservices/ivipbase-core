...
const bigIntFunctions = {
	...
	bigintToBytes(number: bigint): number[] {
		throw new Error(noBigIntError);
	},
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	bytesToBigint(bytes: TypedArrayLike | TypedArray | number[]): bigint {
		throw new Error(noBigIntError);
	},
};
if (hasBigIntSupport) {
...
export function compareValues(oldVal: any, newVal: any, sortedResults = false): ValueCompareResult {
	...
	if (oldVal === newVal) {
		...
		const getKeys = (obj: any) => {
			...
			if (isArray) {
				keys = keys.map((v: ObjectProperty) => parseInt(v as string) as ObjectProperty);
			}
			return keys;
		};
