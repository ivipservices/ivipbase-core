...
export class EventSubscription {
    ...
    private _internal: {
        state: "init" | "active" | "canceled";
        cancelReason?: string;
        activatePromises: {
            callback?: (activated: boolean, cancelReason?: string) => void;
            resolve?: () => void;
...
export class EventStream<T = any> {
     ...
     */
    subscribe: <Val = T>(
        callback: (value: Val) => void,
        activationCallback?: (activated: boolean, cancelReason?: string) => void
    ) => EventSubscription;
    ...
    constructor(
        eventPublisherCallback: (eventPublisher: EventPublisher) => void
    ) {
        ...
        this.subscribe = (callback, activationCallback) => {
            if (typeof callback !== "function") {
                ...
                throw new Error(
                    "stream can't be used anymore because all subscribers were stopped"
                );
            }
            ...
            const sub = {
                ...
                activationCallback: function (
                    ...
                    this.subscription._setActivationState(
                        activated,
                        cancelReason
                    );
                },
                subscription: new EventSubscription(function stop() {
                    subscribers.splice(subscribers.indexOf(sub as any), 1);
            ...
            if (typeof activationState !== "undefined") {
                if (activationState === true) {
                    ...
                    sub.subscription._setActivationState(
                        false,
                        activationState
                    );
                }
        ...
        const publish = (val: T) => {
            subscribers.forEach((sub) => {
                try {
                    ...
                    console.error(
                        `Error running subscriber callback: ${err.message}`
                    );
                }
            });
            if (subscribers.length === 0) {
        ...
        const cancel = (reason: string) => {
            ...
            subscribers.forEach((sub) => {
                sub.activationCallback?.(
                    false,
                    reason || new Error("unknown reason")
                );
            });
