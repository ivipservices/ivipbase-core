 ...
 */
function get(mappings: ITypeMappings, path: string): ITypeMapping | undefined {
 ...
 */
function map(mappings: ITypeMappings, path: string): ITypeMapping | undefined {
 ...
 */
function mapDeep(
    mappings: ITypeMappings,
    entryPath: string
): { path: string; type: ITypeMapping }[] {
    ...
    const matches = Object.keys(mappings).reduce(
        (
            m: Array<{
                path: string;
                type: ITypeMapping;
...
function process(
    ...
    m.sort((a, b) =>
        PathInfo.getPathKeys(a.path).length >
        PathInfo.getPathKeys(b.path).length
            ? -1
            : 1
    ); // Deepest paths first
    m.forEach((mapping) => {
        ...
        const process = (
            ...
            const key = keys[0];
            let children: { key: string | number; val: any }[] = [];
            ...
            children.forEach((child) => {
                ...
                if (keys.length === 1) {
                    ...
                    if (action === "serialize") {
                        ...
                        parent[child.key] = mapping.type.serialize(
                            child.val,
                            ref
                        );
                    } else if (action === "deserialize") {
    ...
    bind(
        ...
        if (typeof options.creator === "undefined") {
            ...
            if (typeof (type as any)[options.creator] === "function") {
                options.creator = (type as any)[
                    options.creator
                ] as CreatorFunction;
            } else {
                throw new TypeError(
                    `${type.name}.${options.creator} is not a function, cannot use it as creator`
                );
            }
        } else if (typeof options.creator !== "function") {
            throw new TypeError(
                `creator for class ${type.name} must be a function, or the name of a static method`
            );
        }
        ...
        this[_mappings][path] = {
            ...
            deserialize(snap) {
                ...
                let obj;
                if (this.creator) {
                    obj = (this.creator as any).call(this.type, snap);
                } else {
            ...
            serialize(obj, ref) {
                if (this.serializer) {
                    obj = (this.serializer as any).call(obj, ref, obj);
