...
export class PathInfo {
    ...
    child(childKey: string | number | Array<string | number>) {
        if (typeof childKey === "string") {
            if (childKey.length === 0) {
                throw new Error(
                    `child key for path "${this.path}" cannot be empty`
                );
            }
            ...
            keys.forEach((key) => {
                ...
                if (key.length > 128) {
                    throw new Error(
                        `child key "${key}" for path "${this.path}" is too long. Max key length is 128`
                    );
                }
                if (key.length === 0) {
                    throw new Error(
    ...
    equals(otherPath: string | PathInfo): boolean {
        const other =
            otherPath instanceof PathInfo ? otherPath : new PathInfo(otherPath);
    ...
    isAncestorOf(descendantPath: string | PathInfo): boolean {
        const descendant =
            descendantPath instanceof PathInfo
                ? descendantPath
    ...
    isDescendantOf(ancestorPath: string | PathInfo): boolean {
        const ancestor =
            ancestorPath instanceof PathInfo
                ? ancestorPath
