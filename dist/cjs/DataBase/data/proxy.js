...
const PathInfo_1 = __importStar(require("../../Lib/PathInfo"));
...
function getTargetRef(ref, target) {
    ...
    const path = PathInfo_1.default.get(ref.path).childPath(target);
    return new reference_1.DataReference(ref.db, path);
}
...
class LiveDataProxy {
    ...
    static async create(ref, options) {
        var _a;
        ref = new reference_1.DataReference(ref.db, ref.path); // Use copy to prevent context pollution on original reference
        let cache, loaded = false;
        let latestCursor = options === null || options === void 0 ? void 0 : options.cursor;
        let proxy;
        const proxyId = ID_1.default.generate(); //ref.push().key;
        // let onMutationCallback: ProxyObserveMutationsCallback;
        // let onErrorCallback: ProxyObserveErrorCallback = err => {
        //     console.error(err.message, err.details);
        // };
        const clientSubscriptions = [];
        const clientEventEmitter = new SimpleEventEmitter_1.default();
        ...
        const subscription = ref.on("mutations", { syncFallback }).subscribe(async (snap) => {
            ...
            const proceed = mutations.every((mutation) => {
                ...
                const changeSnap = new snapshot_1.DataSnapshot(changeRef, mutation.val, false, mutation.prev, snap.context());
        ...
        const pushLocalMutations = async () => {
            ...
            process.nextTick(() => {
                ...
                mutations.forEach((mutation) => {
                    ...
                    const mutationSnap = new snapshot_1.DataSnapshot(mutationRef, mutation.value, false, mutation.previous, context);
                    // onMutationCallback(mutationSnap, false);
                    clientEventEmitter.emit("mutation", { snapshot: mutationSnap, isRemote: false });
                });
                // }
                // Notify local subscribers
                const snap = new snapshot_1.MutationsDataSnapshot(ref, mutations.map((m) => ({ target: m.target, val: m.value, prev: m.previous })), context);
            ...
            processPromise = mutations
                ...
                await update.ref
                    ...
                    if (update.type === "set") {
                        ...
                        const mutationSnap = new snapshot_1.DataSnapshot(update.ref, update.previous, false, update.value, context);
                        clientEventEmitter.emit("mutation", { snapshot: mutationSnap, isRemote: false });
                        ...
                        Object.keys(update.previous).forEach((key) => {
                            ...
                            const mutationSnap = new snapshot_1.DataSnapshot(update.ref.child(key), update.previous[key], false, update.value[key], context);
                            clientEventEmitter.emit("mutation", { snapshot: mutationSnap, isRemote: false });
                    ...
                    mutations.forEach((m) => {
                        ...
                        const mutationSnap = new snapshot_1.DataSnapshot(mutationRef, m.val, false, m.prev, context);
                        clientEventEmitter.emit("mutation", { snapshot: mutationSnap, isRemote: false });
                    });
                    // Notify local subscribers:
                    const snap = new snapshot_1.MutationsDataSnapshot(update.ref, mutations, context);
                    localMutationsEmitter.emit("mutations", { origin: "local", snap });
                });
        ...
        const localMutationsEmitter = new SimpleEventEmitter_1.default();
        ...
        const reload = async () => {
            ...
            mutations.forEach((m) => {
                ...
                const newSnap = new snapshot_1.DataSnapshot(targetRef, m.val, m.val === null, m.prev, context);
                clientEventEmitter.emit("mutation", { snapshot: newSnap, isRemote: true });
            });
            // }
            // Notify local subscribers
            const mutationsSnap = new snapshot_1.MutationsDataSnapshot(ref, mutations, context);
...
class OrderedCollectionProxy {
    ...
    add(item, index, from) {
        ...
        if (typeof from === "number") {
            ...
            if (typeof index === "number" && Math.abs(from - index) === 1) {
    ...
    delete(index) {
        ...
        delete this.collection[key]; // Deletes it from db
        return { key, index };
    }
    /**
    ...
    const handler = {
        get(target, prop, receiver) {
            ...
            if (["string", "number", "boolean"].includes(typeof value) ||
                ...
                value instanceof PathInfo_1.PathReference ||
